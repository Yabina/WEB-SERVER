...
exports.GET_ME_BY_USER_ID = (userId) =>`SELECT user_id, username, email FROM users WHERE user_id = ${userId}`; // don't return the password

exports.GET_ME_BY_USERNAME = (username) => `SELECT user_id, username, email FROM users WHERE username = ${username}`; // don't return the password

exports.GET_ME_BY_USER_ID_WITH_PASSWORD = (userId) => `SELECT * FROM users WHERE user_id = ${userId}`; 
